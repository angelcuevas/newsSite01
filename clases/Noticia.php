...
class Noticia {
	...
	public function tapa(){
        ...
        $noticias = $this->db->query("SELECT noticias.hits,noticias.id_categoria,	noticias_tapa.Columna, noticias_tapa.ubicacion,	noticias.id_noticia, noticias.volanta,	noticias.copete,	noticias.titulo,	noticias.fecha,	noticias.activa	FROM noticias_tapa INNER JOIN noticias ON  noticias_tapa.Id_Noticia = noticias.id_Noticia ORDER BY ubicacion ASC");
        
        return $noticias;
    }
    
    public function listadoPorCategoriaUltimasCinco($id_categoria){
    ...
    public function getLastNews(){
        ...
        $noticias = $this->db->query("SELECT noticias.id_noticia, noticias.titulo, (SELECT noticias_fotos.url FROM noticias_fotos WHERE noticias_fotos.id_noticia = noticias.id_noticia LIMIT 1 ) as foto FROM noticias WHERE DAY(NOW())-30 < DAY(noticias.fecha) ORDER BY fecha DESC LIMIT 5");
